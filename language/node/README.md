## NodeJS

### Tips

- [Package.json과 Package-lock.json의 차이를 아시나요?](https://velog.io/@songyouhyun/Package.json%EA%B3%BC-Package-lock.json%EC%9D%98-%EC%B0%A8%EC%9D%B4)
- [NodeJS - JWT Token 사용하기](https://velog.io/@neity16/NodeJS-JWT-Token-%EC%82%AC%EC%9A%A9%ED%95%98%EA%B8%B0)
- [Google OAuth를 활용한 로그인 페이지 만들기](https://yohanpro.com/posts/codereview)
- [PM2를 활용한 Node.js 무중단 서비스하기](https://engineering.linecorp.com/ko/blog/pm2-nodejs/)

### Fixes

- 실행에러문제 `Node Sass does not yet support your current environment: Windows 64-bit with Unsupported runtime`
    - [https://kcmschool.com/175](https://kcmschool.com/175)
    - [felixrieseberg/windows-build-tools#116](https://github.com/felixrieseberg/windows-build-tools/issues/116)
